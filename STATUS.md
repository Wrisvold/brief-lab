...
Since then: Ward ran the app live with a real key (List models, Test connection, runs, and the
student features all worked), and the Firefox / Safari / Edge engines were checked by script (see
above). The separate checklist for John was removed on 2026-09-17 because nothing left on it needed a
second person.

**The recorded walkthrough is in.** `data/walkthrough-recorded.json` was generated by the app on
2026-09-17 from eight live runs on **Claude Haiku 4.5** (`claude-haiku-4-5`, temperature 0.7) and
saved with the instructor button; validated with the app's own reader (all eight stops usable, 124 to
221 words each). An earlier recording on OpenAI gpt-4o-mini was replaced the same day because the
classes use Claude. Students without a key can now replay the whole walkthrough, labelled as recorded
on every output. To re-record after a model change, follow README "Recording the walkthrough runs".
Live calls have now been made successfully on OpenAI and on Claude.

Nothing from the build brief remains open.
