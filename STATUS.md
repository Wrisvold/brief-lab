...
2026-09-17 from eight live runs (OpenAI gpt-4o-mini, temperature 0.7) and saved with the instructor
button; validated with the app's own reader (all eight stops usable, 106 to 202 words each). Students
without a key can now replay the whole walkthrough, labelled as recorded on every output. To re-record
on Gemini or after a model change, follow README "Recording the walkthrough runs".
